 ...
 *
 * The stock HID host only drives the IN endpoint + control SET_REPORT. The
 * Skylanders Trap Team portal uses its interrupt OUT endpoint (0x02) for the
 * speaker audio and trap-LED effects, which must go to that endpoint and NOT be
 * mis-parsed as control commands. Transfer is fire-and-forget; if the previous
 * one is still in flight the new data is dropped (audio is loss-tolerant).
 */
